...
export function AiQuizWorkspace({
  ...
  const [result, setResult] = useState<{
    score: number;
    reviews: GradedReview[];
  } | null>(null);

  const wrong = useMemo(
    () => result?.reviews.filter((r) => !r.isCorrect) ?? [],
    [result],
  );
  ...
  async function generate() {
    ...
    try {
      ...
      if (!res.ok) {
        setError(
          body && "error" in body
            ? (body.error ?? "Could not generate quiz.")
  ...
  return (
    <div className="space-y-6">
      <div
        className={
          embedded
            ? "border-b border-[#ececec] pb-6"
            : "rounded-xl border border-[var(--border)] bg-gradient-to-br from-[var(--surface)] to-[var(--surface-muted)] p-6 shadow-[var(--shadow-md)]"
        }
      >
      ...
      {attempt ? (
        <div className="space-y-4 rounded-xl border border-[var(--border)] bg-[var(--surface)] p-5 shadow-[var(--shadow-sm)]">
          ...
          {attempt.questions.map((q, i) => (
            <div
              key={q.id}
              className="rounded-lg border border-[var(--border)] bg-[var(--background)] p-4 shadow-[var(--shadow-sm)]"
            >
