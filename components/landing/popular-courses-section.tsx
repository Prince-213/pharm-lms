...
export function PopularCoursesSection({
  ...
  const courses = programs.pages[active] ?? programs.pages[0];
    ...
    <section className="bg-slate-50/50 py-16 lg:py-20">
      <div className="mx-auto w-[90%] lg:w-[80%] px-4 sm:px-6 lg:px-10">
        <div className="mb-10">
          ...
          <p className="mt-3 max-w-lg text-[var(--muted-soft)]">{programs.description}</p>
        </div>
        ...
        {programs.pages.length > 1 && (
          <div className="mt-8 flex justify-center gap-2">
            {programs.pages.map((_, i) => (
              <button
                key={i}
                type="button"
                onClick={() => setActive(i)}
