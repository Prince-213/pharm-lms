...
export default async function MentorMessagesPage({
  ...
  if (!session?.user) redirect("/mentor/login?callbackUrl=/mentor/messages");
    ...
    <div className="space-y-6 text-[var(--foreground)]">
      <div>
        ...
        <p className="mt-1 text-sm text-[var(--muted)]">
          Conversations with students and admins. New messages appear here
          automatically.
        </p>
      </div>

      <div className="flex min-h-[520px] flex-col overflow-hidden rounded-[var(--radius-lg)] border border-[var(--border)] bg-[var(--surface)] shadow-[var(--shadow-sm)] md:flex-row">
          ...
          {threads.length === 0 ? (
            <div className="flex flex-1 flex-col items-center justify-center px-6 py-16 text-center">
              ...
              <p className="mt-2 max-w-[240px] text-xs text-[var(--muted)]">
                When students or admins reach out, your conversations will show
                up here.
              </p>
            </div>
          ) : (
          ...
          {active ? (
            <>
              ...
              <div className="border-t border-[var(--border)] bg-[var(--background)] px-4 py-3">
                <ThreadComposer threadId={active.thread.id} placeholder="Reply…" />
              </div>
            </>
          ) : (
            <div className="flex min-h-[280px] flex-1 flex-col items-center justify-center px-6 py-16 text-center text-sm text-[var(--muted)]">
