...
export default async function StudentMentorDetailPage({
  ...
  if (!courseId) redirect("/student/meetings");
  ...
  if (!enrollment || enrollment.course.mentorId !== mentorId) {
    redirect("/student/meetings");
  }

  const [mentor, learnersReached, publishedCourses] = await Promise.all([
    db.user.findFirst({
  ...
  const bioText =
    mentor.bio?.trim() ||
    "This mentor supports PharmLms learners with structured discussions, exam preparation, and practical clinical reasoning.";
  ...
  const clinicalBlurb =
    sentences[0] ??
    `Leads instruction and mentoring for ${enrollment.course.title}, with a focus on clear outcomes and applied competence.`;
  const researchBlurb =
    sentences.slice(1, 3).join(" ").trim() ||
    "Hosts live sessions, answers case-based questions, and keeps material aligned with current practice expectations.";

  const tagline =
    enrollment.course.subtitle?.trim() ||
    `Principal mentor for ${enrollment.course.title}. Former clinical educator supporting PharmLms cohorts.`;
      ...
      <div className="grid gap-8 lg:grid-cols-[minmax(0,1fr)_min(100%,22rem)] lg:items-start">
        ...
        <aside className="lg:sticky lg:top-6">
          <div className="rounded-[var(--radius-xl)] border border-[var(--border)] bg-[var(--surface)] p-6 shadow-[var(--shadow-md)]">
            ...
            <p className="mt-2 text-xs leading-relaxed text-[var(--muted)]">
              One-on-one session aligned with your enrolled course. Pick a slot
              that fits your mentor’s weekly availability.
            </p>
            ...
            <ul className="mt-6 space-y-4 text-sm">
              ...
              <li className="flex flex-col gap-0.5 pb-2">
                ...
                <span className="text-[11px] text-[var(--muted-soft)]">
                  Consultations are included with your enrollment; no extra
                  charge at booking.
                </span>
              </li>
            </ul>
            ...
            <div className="mt-6">
              {mentor.availability.length ? (
                ...
                <p className="rounded-[var(--radius-lg)] border border-dashed border-[var(--border)] bg-[var(--surface-muted)] px-4 py-4 text-center text-xs font-medium text-[var(--muted)]">
                  This mentor has not set weekly hours yet. Check back soon or
                  message them from your course.
                </p>
              )}
            </div>
          </div>
